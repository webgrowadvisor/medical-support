                ...
                <div class="page-header-right ms-auto">
                    <div class="page-header-right-items">
                        ...
                        <div class="d-flex align-items-center gap-2 page-header-right-items-wrapper">
                            ...
                            <a href="{{ route('admin.commission.plans.create') }}" class="btn btn-primary">
                                    ...
                                    <table class="table table-hover" id="leadList">
                                        ...
                                        <tbody>
                                            @foreach($plans as $key => $plan)
                                                <tr>
                                                    <td>{{ $key+1 }}</td>
                                                    <td>{{ $plan->specialization ?? 'All' }}</td>
                                                    <td>{{ $plan->doctor_id ? 'Doctor #' . $plan->doctor_id : 'All Doctors' }}</td>
                                                    <td>{{ ucfirst($plan->type) }} </td>
                                                    <td>
                                                        @if($plan->type == 'percentage')
                                                        {{ number_format($plan->commission_value, 0) }}%
                                                        @else
                                                        {{ priceicon() }}{{ $plan->commission_value }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('admin.commission.plans.edit', $plan->id) }}"
