                                        ...
                                        <tbody>
                                            @forelse($payments as $key => $payment)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    <td>
                                                        @if($payment->wallet_type === 'user')
                                                            👤 {{ $payment->user?->name }}
                                                        @elseif($payment->wallet_type === 'doctor')
                                                            🧑‍⚕️ {{ $payment->doctor?->name }}
                                                        @else
                                                            Admin
                                                        @endif
                                                    </td>
                                                    <td>
                                                        <span class="badge bg-{{ $payment->type == 'credit' ? 'success' : 'danger' }}">
                                                            {{ strtoupper($payment->type) }}
                                                        </span>
                                                    </td>
                                                    <td>{{ priceicon() }}{{ number_format($payment->amount, 2) }}</td>
                                                    <td>{{ priceicon() }}{{ number_format($payment->balance, 2) }}</td>
